...
export default async function DashboardPage() {
  ...
  // Shared card style via inline CSS variables
  const card = {
  ...
  return (
    <div style={{ minHeight: '100%', padding: 24, display: 'flex', flexDirection: 'column', gap: 20, background: 'var(--bg)' }}>

      {/* Hero card */}
      <div style={{ ...heroCard, padding: 24 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24, flexWrap: 'wrap' }}>
          {/* Left: value + stats */}
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-3)', marginBottom: 8, fontFamily: 'var(--font-space-mono)' }}>
              Valeur totale du portefeuille
            </p>
            <p style={{ fontSize: 36, fontWeight: 700, color: 'var(--text)', fontFamily: 'var(--font-syne)', marginBottom: 12, lineHeight: 1.1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
              {totalValue > 0 && (
                <>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <i className={`ti ti-trending-${totalPL >= 0 ? 'up' : 'down'}`} style={{ fontSize: 15, color: totalPL >= 0 ? 'var(--green)' : 'var(--red)' }} />
                    <span style={{ fontSize: 14, fontWeight: 600, color: totalPL >= 0 ? 'var(--green)' : 'var(--red)', fontFamily: 'var(--font-space-mono)' }}>
                      {fmt(totalPL)}
                    </span>
                    <span style={{
                      fontSize: 11,
                      ...
                      background: totalPL >= 0 ? 'rgba(52,199,142,0.12)' : 'rgba(240,85,85,0.12)',
              ...
              <div>
                <span style={{ fontSize: 12, color: 'var(--text-3)' }}>Investi </span>
                <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', fontFamily: 'var(--font-space-mono)' }}>
                  {totalInvested > 0 ? fmt(totalInvested) : '—'}
                </span>
              </div>
              <div>
                <span style={{ fontSize: 12, color: 'var(--text-3)' }}>Positions </span>
                <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--accent)', fontFamily: 'var(--font-space-mono)' }}>
                  {positions.length}
                </span>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
            <p style={{ fontSize: 10, color: 'var(--text-3)', fontFamily: 'var(--font-space-mono)', textAlign: 'right' }}>
              Investi · 7 mois
            </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
        ...
        <div style={{ ...card, padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-3)', fontFamily: 'var(--font-space-mono)' }}>
              Positions actives
            </p>
          ...
          {positions.length === 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 0', textAlign: 'center', gap: 8 }}>
              <p style={{ fontSize: 13, color: 'var(--text-3)' }}>Aucune position ouverte</p>
              <Link href="/portfolio" style={{ fontSize: 12, fontWeight: 600, color: 'var(--accent)', textDecoration: 'none' }}>
                Commencer →
              </Link>
            </div>
          ) : (
                ...
                <div key={p.symbol} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 10px', borderRadius: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: 11, color: 'var(--text-3)', margin: 0, fontFamily: 'var(--font-space-mono)' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <p style={{ fontSize: 11, fontWeight: 600, color: p.plPct >= 0 ? 'var(--green)' : 'var(--red)', margin: 0, fontFamily: 'var(--font-space-mono)' }}>
        ...
        <div style={{ ...card, padding: 20 }}>
          ...
          {activeAlerts.length === 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 0', textAlign: 'center', gap: 8 }}>
              <p style={{ fontSize: 13, color: 'var(--text-3)' }}>Aucune alerte configurée</p>
              <Link href="/agent" style={{ fontSize: 12, fontWeight: 600, color: 'var(--accent)', textDecoration: 'none' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {activeAlerts.map((alert) => (
                <div key={alert.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', borderRadius: 10, background: 'var(--accent-dim)', border: '1px solid var(--border)' }}>
                  <i className="ti ti-bell" style={{ fontSize: 15, color: 'var(--accent)', marginTop: 1, flexShrink: 0 }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <form action={deleteAlert}>
                    ...
                    <button type="submit" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', display: 'flex', alignItems: 'center', padding: 2, borderRadius: 4 }} title="Supprimer">
      ...
      <div style={{ ...card, padding: 20 }}>
        ...
        {recentTrades.length === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 0', gap: 8 }}>
            <p style={{ fontSize: 13, color: 'var(--text-3)' }}>Aucune transaction pour le moment</p>
            <Link href="/portfolio" style={{ fontSize: 12, fontWeight: 600, color: 'var(--accent)', textDecoration: 'none' }}>
              ...
              <thead>
                <tr>
                  {['Type', 'Symbole', 'Quantité', 'Prix', 'Valeur', 'Date'].map((col) => (
                    <th key={col} style={{ textAlign: 'left', paddingBottom: 12, paddingRight: 16, fontSize: 11, fontWeight: 700, color: 'var(--text-3)', fontFamily: 'var(--font-space-mono)', whiteSpace: 'nowrap' }}>
                      {col}
                    </th>
                  ))}
                </tr>
                  ...
                  <tr key={trade.id} style={{ borderTop: i > 0 ? '1px solid var(--border)' : 'none' }}>
                    <td style={{ padding: '12px 16px 12px 0' }}>
                      <span style={{
                        ...
                        fontSize: 11,
                        fontWeight: 700,
                        background: trade.type === 'BUY' ? 'var(--accent-dim)' : 'rgba(240,85,85,0.12)',
                        color: trade.type === 'BUY' ? 'var(--accent)' : 'var(--red)',
                      }}>
                    ...
                    <td style={{ padding: '12px 16px 12px 0', fontSize: 13, color: 'var(--text)', fontFamily: 'var(--font-space-mono)' }}>
                      {trade.quantity.toFixed(4)}
                    </td>
                    <td style={{ padding: '12px 16px 12px 0', fontSize: 13, color: 'var(--text)', fontFamily: 'var(--font-space-mono)' }}>
                      ${trade.price.toFixed(2)}
                    </td>
                    <td style={{ padding: '12px 16px 12px 0', fontSize: 13, fontWeight: 600, color: 'var(--text)', fontFamily: 'var(--font-space-mono)' }}>
                      ${(trade.price * trade.quantity).toFixed(2)}
                    </td>
                    <td style={{ padding: '12px 0', whiteSpace: 'nowrap' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--text-3)', fontFamily: 'var(--font-space-mono)' }}>
      ...
      {/* Spacer for floating chat button */}
      <div style={{ height: 80 }} />
    </div>
  );
}
